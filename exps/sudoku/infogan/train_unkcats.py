...
from models.mnist_unkcats import Generator, Discriminator, DHead, QHead
...
num_cats = 16
save = True
checkpoint_dir = Path(os.path.join(Path(__file__).parent, 'num_cats_{}'.format(str(num_cats))))
...
plt.imshow(np.transpose(vutils.make_grid(
    sample_batch[0].to(device)[ : 100], nrow=num_cats, padding=2, normalize=True).cpu(), (1, 2, 0)))
plt.savefig('Training Images {}'.format(params['dataset']))
plt.close('all')

# Initialise the network.
netG = Generator(num_cats).to(device)
...
netQ = QHead(num_cats=num_cats).to(device)
...
# Fixed Noise
z = torch.randn(num_cats * 10, params['num_z'], 1, 1, device=device)
fixed_noise = z
if(params['num_dis_c'] != 0):
    idx = np.arange(params['dis_c_dim']).repeat(10)
    dis_c = torch.zeros(num_cats * 10, params['num_dis_c'], params['dis_c_dim'], device=device)
    for i in range(params['num_dis_c']):
        dis_c[torch.arange(0, num_cats * 10), i, idx] = 1.0

    dis_c = dis_c.view(num_cats * 10, -1, 1, 1)

    fixed_noise = torch.cat((fixed_noise, dis_c), dim=1)

if(params['num_con_c'] != 0):
    con_c = torch.rand(num_cats * 10, params['num_con_c'], 1, 1, device=device) * 2 - 1
...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, _) in enumerate(dataloader, 0):
        ...
        for j in range(params['num_dis_c']):
            dis_loss += criterionQ_dis(q_logits[:, j*num_cats : j*num_cats + num_cats], target[j])
    ...
    img_list.append(vutils.make_grid(gen_data, nrow=num_cats, padding=2, normalize=True))
    ...
    if((epoch+1) == 1 or (epoch+1) == params['num_epochs']/2):
        ...
        plt.figure(figsize=(10, 10))
        plt.axis("off")
        plt.imshow(np.transpose(vutils.make_grid(gen_data, nrow=num_cats, padding=2, normalize=True), (1,2,0)))
        plt.savefig("Epoch_%d {}".format(params['dataset']) %(epoch+1))
        plt.close('all')

    # Save network weights.
    if (epoch+1) % params['save_epoch'] == 0:
        values, indices, num_datapoints = get_mapping(lambda x: netQ(discriminator(x)), device, params, num_cats=num_cats)
