...
import satnet
import infogan.models.mnist_unkcats as mnist_infogan
import infogan.models.fashionmnist_model as fashion_infogan


class SudokuSolver(nn.Module):
    def __init__(self, boardSz, aux, m, leak_labels, softmax, pass_mask=False, num_cats=9):
    ...
    def forward(self, y_in, mask):
        ...
        if self.softmax:
            out = torch.softmax(out.contiguous().view(-1, self.num_cats), dim=1).view(-1, in_.shape[1])
            # out = torch.softmax(out.contiguous().view(-1, 9), dim=1).view(-1, 729)
...
class DigitConv(nn.Module):
    ...
    def __init__(self, num_cats=9):
        super(DigitConv, self).__init__()
        self.conv1 = nn.Conv2d(1, 20, 5, 1)
        self.conv2 = nn.Conv2d(20, 50, 5, 1)
        self.fc1 = nn.Linear(4 * 4 * 50, 500)
        self.fc2 = nn.Linear(500, num_cats + 1)
        self.num_cats = num_cats
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return F.softmax(x, dim=1)[:, :self.num_cats].contiguous()
...
class MNISTSudokuSolver(nn.Module):
    def __init__(self, boardSz, aux, m, leak_labels, softmax, proofread=False, num_cats=9):
        # print(num_cats)
        # exit()
        super(MNISTSudokuSolver, self).__init__()
        self.num_cats = num_cats
        self.digit_convnet = DigitConv(num_cats)
...
class InfoGanSudokuSolver(nn.Module):
    def __init__(self, boardSz, aux, m, leak_labels, fashion=False, num_cats=9):
        super(InfoGanSudokuSolver, self).__init__()

        self.num_cats = num_cats
        infogan_module = mnist_infogan
        self.discriminator = infogan_module.Discriminator()
        self.qnet = infogan_module.QHead(num_cats)
        self.sudoku_solver = SudokuSolver(boardSz, aux, m, leak_labels, softmax=True, pass_mask=False, num_cats=num_cats)
        self.boardSz = boardSz
        self.nSq = boardSz
    ...
    def get_pieces(self):
        return {**self.sudoku_solver.get_pieces(), 'discriminator': self.discriminator.state_dict(),
                'netQ': self.qnet.state_dict(), 'current_perm': self.current_perm}
